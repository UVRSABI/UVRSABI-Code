...
class AdjacentRoofDistance:
    ...
    def ClusterAlongXaxis(self,cloud):
        ...
        leftBuildingPoints=newpoints[newpoints[:,0]< -0.1]
        rightBuildingPoints=newpoints[newpoints[:,0]>0.1]
    ...
    def EstimateDistanceAdjacentBuildings(self):
        ...
        '''
        final_pts = np.concatenate((leftBuildingpts, rightBuildingpts), axis=0)
        print(lefBuildingpts.shape, rightBuildingpts.shape, final_pts.shape)
        # exit()
        newcloud = o3d.geometry.PointCloud()
        newcloud.points = o3d.utility.Vector3dVector(final_pts)
        ...
        for i in range(20):
            ...
            # o3d.visualization.draw_geometries([rest,segments[i]])
            rest = rest.select_by_index(inliers, invert = True)
            print("pass",i,"/",20,"done.")
        o3d.visualization.draw_geometries([segments[i] for i in range(20)])
        exit()
        '''

        # OBTAINING THE LEFT AND RIGHT BUILDINGS SEGMENTED IN Z-AXIS
        '''
        print(np.array(leftBuilding.points).shape)
        points = np.array(leftBuilding.points)
        mypt = points[1000]
        dist_list = []
        # for j in range(679887)
        for i in range(679887):
            pt = points[i]
            dist = ((pt[0] - mypt[0])**2 + (pt[1] - mypt[1])**2 + (pt[2] - mypt[2])**2)**(1/2)
            dist_list.append((dist,i))
        dist_list.sort()
        for i in range(100):
            print(i, dist_list[i])
        # print( dist_list[0:100])
        # exit()
        # exit()
        colors = np.array(leftBuilding.colors)
        
        colors[1000] =[1,0,0]
        colors[dist_list[1][1]] =[0, 0, 1] 
        leftBuilding.colors = o3d.utility.Vector3dVector(colors)
        o3d.visualization.draw_geometries([leftBuilding])
        exit()
        labels = np.array(cloud.cluster_dbscan(eps=0.01, min_points = 100, print_progress = True))
